...
WF1_INPUT_DIR = WF2_INPUT_DIR = WF3_INPUT_DIR = WF4_INPUT_DIR = ""
OUT_DIR = ANN_DIR = ""
WF1_MODELS = [
...
def configure_output_paths(out_dir: str) -> None:
    """
    Configure the output directory layout at runtime.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run the clinical NER pipeline over a directory of Spanish clinical .txt notes."
    )
    parser.add_argument("--input-dir", required=True, help="Directory containing input .txt files.")
    parser.add_argument(
        "--out-dir",
        required=True,
        help="Directory where CSV, JSON and .ann outputs will be written.",
    )
...
if __name__ == "__main__":
    args = parse_args()
    WF1_INPUT_DIR = WF2_INPUT_DIR = WF3_INPUT_DIR = WF4_INPUT_DIR = args.input_dir
    configure_output_paths(args.out_dir)
